...
type statsPair struct {
	...
	Text  string `fesl:"text,omitempty"` //TEST
...
func (r *Ranking) getStats(event *network.EventClientCommand) {
	...
	if heroID == 0 {
		if event.Client.Type == "server" {
			...
			// hero
			player, err := r.DB.FindPlayerByID(r.DB.NewSession(), ownerID)
			if err != nil {
				logrus.
			...
			if player.SelectedHeroID.Valid == false {
				logrus.
					WithField("heroID", heroID).
					WithField("owner", event.Command.Message["owner"]).
					WithField("player", player.ID).
					WithField("selectedHeroID", player.SelectedHeroID).
					WithField("cmd", "rank.GetStats").
					Warnf("Player has unset selected hero")
				return
			}

			logrus.Warnf("GetStats (client), replacing ownerID with heroID with %v", player.SelectedHeroID.Int64)
			heroID = int(player.SelectedHeroID.Int64)
		}
	}
