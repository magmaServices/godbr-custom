...
func main() {
	...
	flag.StringVar(&pkgScan, "scan", "./model", "Path to directory which should be scanned for the *.go files.")
	flag.StringVar(&getOut, "getters", "./inter/ranking/getters.go", "Output file for getters")
	flag.StringVar(&setOut, "setters", "./inter/ranking/setters.go", "Output file for setters")
	flag.StringVar(&addOut, "adders", "./inter/ranking/adders.go", "Output file for adders")
	flag.Parse()
...
func (w *Walker) analyzeStruct(str *ast.StructType) {
	for _, field := range str.Fields.List {
		...
		if w.IsEmbededField(typ) {
			log.Printf("tag: field with type %v is embedded, omiting...", typ.String())
			continue
		}
		...
		if err != nil {
			log.Printf("tag: field with name %s does not have tag '%s', omiting...", field.Names[0], tagName)
			continue
		}
